...
version (Tango) {
  ...
  import tango.stdc.stringz : toCString = toUtf8z;
  debug (UnitTest) import std.io.Console;
} else {
  import std.string : toDString = toString;
  import std.string : toCString = toStringz;
  debug (UnitTest) import std.stdio;
}
import dbi.Database, dbi.DBIException, dbi.Result, dbi.Row;
import dbi.mssql.imp, dbi.mssql.MssqlResult; // , dbi.mssql.MssqlError;
...
public:
  ...
  override void connect (char[] params, char[] username = null, char[] password = null) {
    ...
    // propset username
    ret = ct_con_props(con, CS_SET, CS_USERNAME, toCString(username), 
		       CS_NULLTERM, null);
    if ( ret != CS_SUCCEED ) 
      throw new DBIException("Cannot set 'username' connection property");
    
    // propset password
    ret = ct_con_props(con, CS_SET, CS_PASSWORD, toCString(password), 
		       CS_NULLTERM, null);
    if ( ret != CS_SUCCEED ) 
      throw new DBIException("Cannot set 'password' connection property");

    // propset serveraddr (host, port)
    ret = ct_con_props(con, CS_SET, CS_SERVERADDR, toCString(params), 
		       CS_NULLTERM, null);
    if ( ret != CS_SUCCEED ) 
  ...
  override void execute (char[] sql) {
    ...
    if ( ct_command(cmd, CS_LANG_CMD, toCString(sql), CS_NULLTERM, 
...
unittest {
  ...
  assert (row !is null);
  assert (row.getFieldIndex("testStr") == 0);
  assert (row.getFieldIndex("testInt") == 1);
  assert (row.getFieldIndex("anotherStr") == 2);
  assert (row.get("testStr") == "abcd");
  // assert (row.get("name") == "");
