  ...
  override Row fetchRow () {
	...
	while (ct_fetch(cmd, CS_UNUSED, CS_UNUSED, CS_UNUSED, &count) == 
	  ...
	  for (i = 0; i < numFields; ++i) {
	    fieldname = fields[i].name[0..find(fields[i].name,'\0')];
...
private:
  ...
  void setNumFields() {
    ...
    fields.length = lengths.length = inds.length 
      = strings.length = floats.length = ints.length =dts.length = dt4s.length 
      = numFields;
  }
