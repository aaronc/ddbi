...
/*############################################
##                                          ##
## ODBC integration for DDBI (odbcResult)   ##
##                                          ##
##         Stay sharp if you edit!          ##
##                                          ##
## 2006: MPSD                               ##
##                                  	    ##
############################################*/
...
import std.string;
import dbi.BaseResult, dbi.Row, dbi.odbc.imp;
...
class odbcResult : BaseResult {
	...
	this(HSTMT hstmt) {
 	...
 	Row fetchRow() {
		...
		SQLCHAR[] column_names[ODBC_COLUMN_MAX];			// Query Field Names
	    ...
	    if ( rc == SQL_SUCCESS ) {
	    	...
	    	for (int i = 0; i < num_cols; i++)
		 		r.addField(	column_names[i].dup, 
		 				std.string.toString(buffer[i]).dup,
		 				std.string.toString("").dup, 0); // No extra goodies :(
